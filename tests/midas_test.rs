...
async fn test_midas() -> Result<()> {
    ...
    let block_count = results
        .getblockcount
        .expect("getblockcount was in the batch");
    let net_info = results
        .getnetworkinfo
        .expect("getnetworkinfo was in the batch");
    let difficulty = results
        .getdifficulty
        .expect("getdifficulty was in the batch");
    let conn_count = results
        .getconnectioncount
        .expect("getconnectioncount was in the batch");
    ...
    let balance_amount = client
        .getreceivedbyaddress(address.clone(), 0, false)
        .await?
        .0;
    ...
    let hex = finalized_obj
        .hex
        .ok_or_else(|| anyhow!("Failed to get hex from finalized PSBT"))?;
